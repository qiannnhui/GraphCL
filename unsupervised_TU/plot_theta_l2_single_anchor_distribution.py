...
def plot_theta_l2_distribution(x, x_aug, labels=None, args=None, epoch=None):
    ...
    total_fn_l2_count = 0
    total_fn_theta_count = 0
    total_fn_l2_count_neg = 0
    total_fn_theta_count_neg = 0
    total_tn_l2_count = 0
    total_tn_theta_count = 0
    total_tn_l2_count_neg = 0
    total_tn_theta_count_neg = 0
    ...
    for anchor_id in range(x.size(0)):
        ...
        # === False Negatives 比 GCL Pos 小的數量 ===
        fn_l2_count_neg = np.sum(real_neg_l2 < gcl_pos_l2_mean)
        fn_theta_count_neg = np.sum(real_neg_theta < gcl_pos_theta_mean)
        ...
        # === True Negatives (GCL Negatives) 比 GCL Pos 小的數量 ===
        tn_l2_count_neg = np.sum(gcl_neg_l2 < gcl_pos_l2_mean)
        tn_theta_count_neg = np.sum(gcl_neg_theta < gcl_pos_theta_mean)

        total_fn_l2_count += fn_l2_count
        total_fn_theta_count += fn_theta_count
        total_fn_l2_count_neg += fn_l2_count_neg
        total_fn_theta_count_neg += fn_theta_count_neg
        total_tn_l2_count += tn_l2_count
        total_tn_theta_count += tn_theta_count
        total_tn_l2_count_neg += tn_l2_count_neg
        total_tn_theta_count_neg += tn_theta_count_neg
        
    return {
         'anchor_id': anchor_id,
         'fn_l2_count': total_fn_l2_count,
         'fn_theta_count': total_fn_theta_count,
         'fn_l2_count_neg': total_fn_l2_count_neg,
         'fn_theta_count_neg': total_fn_theta_count_neg,
         'tn_l2_count': total_tn_l2_count,
         'tn_theta_count': total_tn_theta_count,
         'tn_l2_count_neg': total_tn_l2_count_neg,
         'tn_theta_count_neg': total_tn_theta_count_neg
    }