...
def plot_theta_per_epoch(args, all_epoch_thetas: List[np.ndarray], save_dir=None):
    # 設置輸出目錄
    if save_dir is None:
    ...
    num_epochs = len(all_epoch_thetas)
    ...
    for i, arr in enumerate(all_epoch_thetas):
        if len(arr) > 0:
            counts, _ = np.histogram(arr, bins=bins, range=(angle_min, angle_max))
    ...
    plt.title('Angle Distribution Heatmap Across Epochs', fontsize=16) 
    ...
    heatmap_filename = f'angle_distribution_heatmap.png' 
    ...
    for row_data in stats_list:
        formatted_row = [
            f'{row_data["Epoch"]}',
            f'{row_data["Q1"]:.2f}' if not np.isnan(row_data["Q1"]) else '--',
            f'{row_data["Median (Q2)"]:.2f}' if not np.isnan(row_data["Median (Q2)"]) else '--',
            f'{row_data["Q3"]:.2f}' if not np.isnan(row_data["Q3"]) else '--',
            f'{row_data["Mean"]:.2f}' if not np.isnan(row_data["Mean"]) else '--',
            f'{row_data["Count"]}'
        ]
    ...
    plt.title('Angle Distribution Statistics Per Epoch', fontsize=14) 
    plt.tight_layout()
    
    stats_filename = f'angle_distribution_stats_per_epoch.png' 
