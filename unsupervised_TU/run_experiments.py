#!/usr/bin/env python
"""
GraphCL Multi-Seed Experiment Runner
=====================================
Group A: normal (standard GraphCL InfoNCE)
Group B: reweight_random_FN (randomly drops 50% of ALL negatives)

Each group: 5 seeds × 8 datasets, 100 epochs, log_interval=1.
Picks best-val-acc test acc per run, reports mean ± std per dataset.
For group B, it also records the pFN precision, recall, and f1 at the best val acc epoch.
"""
...
# Config
# ──────────────────────────────────────────────
DATASETS   = ['MUTAG', 'DD', 'IMDB-BINARY', 'REDDIT-BINARY',
               'NCI1', 'PROTEINS', 'COLLAB', 'REDDIT-MULTI-5K']
MODES      = ['normal', 'reweight_random_FN']   # Group A first, then B
SEEDS      = [0, 1, 2, 3, 4]
EPOCHS     = 100
LOG_INT    = 1
LR         = '1e-4'
...
NUM_GC     = 5
BATCH_SIZE = 128
AUG_RATIO  = 1   # -> aug_ratio float = 0.1 inside gsimclr.py

CUDA_DEV   = sys.argv[1] if len(sys.argv) > 1 else '0'
BASE_DIR   = os.path.dirname(os.path.abspath(__file__))
RESULT_BASE = os.path.join(BASE_DIR, 'result', 'GCL')
SUMMARY_FILE = os.path.join(BASE_DIR, 'result', 'experiment_summary.txt')
...
header = (
    '=' * 70 + '\n'
    f'GraphCL Experiment Summary  —  {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}\n'
    f'Epochs={EPOCHS}  LogInterval={LOG_INT}  LR={LR}  Aug={AUG}  Seeds={SEEDS}\n'
    '=' * 70
)
            ...
            try:
                ...
                for line in proc.stdout.splitlines():
                    ...
                    if line.startswith('BEST_FN_METRICS='):
                        try:
                            fn_metrics = json.loads(line.split('=', 1)[1])
                        except:
                            pass
